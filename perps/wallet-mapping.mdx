...
Polymarket has four account/wallet types, and `wallet_type` uses their official names:

| `wallet_type` | What it is | Resolution |
|---------------|-----------|------------|
| `eoa` | A plain private-key wallet, no contract. Some users trade Polymarket this way. | The address is its own perps account. |
| `deposit` | A **deposit wallet** — Polymarket's current wallet type, deployed automatically for new accounts (including the newer beacon-upgradeable variant). | Resolved automatically. |
| `safe` | A Gnosis Safe — the wallet type for accounts that connected a browser wallet like MetaMask. | Resolved automatically. |
| `proxy` | A **Polymarket Proxy** — the wallet type behind email-login accounts. | Not yet resolvable — see below. |
