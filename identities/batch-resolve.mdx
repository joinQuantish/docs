...
Resolve up to 1,000 trading wallets per call against PolyNode's identity index (refreshed daily). Each found identity includes `wallet_type`, `deployed`, and `current`. Answers come from a single indexed query — typical response time is tens of milliseconds regardless of batch size.
