...
<Note>
**Current clarification:** this historical release note described the builder
plan available at that time. Current user-owned execution removes a platform's
shared builder relayer allowance by using wallet-scoped authorization with zero
builder attribution. Normal wallet, market, order API, abuse-prevention, and
rate-limit rules still apply; do not interpret “unlimited” as unbounded order
throughput. See [User-owned execution](/sdks/user-owned-execution).
</Note>
