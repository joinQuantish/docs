...
Responses include `count`, `truncated`, and ordered `ticks`. Each tick includes
a replay-safe `id`, `timestamp_ms`, and the same Chainlink feed, price, bid,
ask, and timestamp values used by the live stream.

If `truncated` is `true`, narrow the time range or increase the limit. Ranges
before collection began return an empty `ticks` array.
