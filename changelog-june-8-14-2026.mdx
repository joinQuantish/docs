...
[Newer: June 15-21, 2026 ←](/changelog-june-15-21-2026) · [Earlier: June 1-7, 2026 →](/changelog-june-1-7-2026)
...
## 2026-06-11 - Chainlink coverage for all seven crypto feeds

Expanded the `price_feed` WebSocket and crypto history APIs to deliver fresh
Chainlink prices for every supported asset.

What's improved:

- `BTC/USD`, `ETH/USD`, `SOL/USD`, `BNB/USD`, `XRP/USD`, `DOGE/USD`, and `HYPE/USD` now receive updates at approximately one tick per second.
...
- `GET /v1/crypto/price`, `GET /v1/crypto/ticks`, and `GET /v1/crypto/candles` provide consistent Chainlink prices across the crypto API.
- New five-minute and fifteen-minute windows receive their opening tick at the window boundary, avoiding the prior freshness delay.
