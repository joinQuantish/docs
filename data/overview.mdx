...
Standard ungrouped list responses also include `rows_returned`, `has_more`, `offset`, `limit`, and `elapsed_ms`. Grouped trade responses are the exception: with `group_by=order_hash`, the response includes `trades`, `rows_returned`, `grouped_by`, and `elapsed_ms`, but omits `has_more`, `offset`, and `limit`.
...
V3 REST data endpoints require a paid API key. Limits are shared across all API keys owned by the same account, so keys can be used at the same time without multiplying the account's throughput.
...
| [Combos](/data/combos/overview) | 8 | Combo markets, P&L rankings, wallet combo positions, combo trades, lifecycle activity, and additive wallet summaries |
...
| [Builders](/data/builders/list) | 6 | Directory and name lookup, profile stats, daily activity and comparison, trader leaderboard, attributed trades |
