...
| `group_by` | string | -- | `user_trade` returns Polymarket-style user-visible trade rows; `transaction_hash` is an alias; `order_hash` aggregates fills per order |
...
Returns one row per order with aggregated totals. This is order-level data, not necessarily one user-visible trade. A single user-visible trade can include multiple order-side rows or complementary outcome rows, so use `group_by=user_trade` when matching Polymarket's wallet trade display.
