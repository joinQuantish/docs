...
| `sort_by` | string | -- | `order_hash` groups fills by limit order, sorted by most recent fill per order |
...
### Group fills by order hash

```bash
curl https://api.polynode.dev/v3/wallets/0xa9857c.../trades?sort_by=order_hash&limit=10
```

Groups all fills belonging to the same limit order together, sorted by most recent order first.
