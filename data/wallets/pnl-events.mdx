...
Returns wallet P&L buckets for charting. With `period`, `after`, or `before`, each row is a fixed time window (hour, day, week, or month) from the realized P&L event stream. A running `cumulative_pnl` is included so the response can be plotted directly.

Without an explicit time filter, the endpoint returns a single all-time summary bucket from the same wallet summary source as [`GET /v3/wallets/{address}/pnl`](/data/wallets/pnl). That default mode is intentionally summary-backed so default `/pnl/events` and `/pnl` agree while the canonical S8 time-series index is being completed.
...
| `source` | string | `wallet_stats_combined` for default summary mode, `pnl_event` for explicit event-series windows |
