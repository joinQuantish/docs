...
| `GET /v3/combos/activity/daily` | Complete UTC-day fills, share volume, lifecycle, transfer, and redemption series |
| `GET /v3/combos/leaderboard` | Eligible-wallet realized P&L windows and marked All Time P&L with explicit exclusions |
...
The [Combo P&L Leaderboard](/data/combos/leaderboard) is stricter than a
single-wallet summary: it ranks only wallets with complete direct cost basis
and every required mark. Wallets with arbitrary transfer-basis gaps or missing
marks are omitted and disclosed, never presented as zero-P&L rows.
