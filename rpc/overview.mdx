...
All keys are rate-limited to **2 HTTP requests per second** per client IP. Exceeding this returns a `-32005` error. A rejected attempt does not extend the limiter window.
