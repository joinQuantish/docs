...
<Info>
  Successfully delivered settlements, status updates, trades, redemptions, oracle events, and related event payloads appear under **On-chain events** in your [usage dashboard](https://polynode.dev/dashboard/usage). Compression reduces counted transfer because the meter uses the encoded payload delivered to the connection. Heartbeats, acknowledgements, control frames, and failed sends do not count. See [Monthly usage allowances](/guides/rate-limits#monthly-usage-allowances).
</Info>
...
| **Combos** | Pending combo trades with order hashes, decoded legs, and receipt confirmations | `{"action":"subscribe","type":"combos"}` |
...
Receive ← {"type": "subscribed", "subscriber_id": "...", "subscription_id": "...:1"}
    ↓
Receive ← {"type": "snapshot", "count": 20, "events": [...]} (recent flat fills)
    ↓
...
function fillTimestampMs(event) {
  // Fill envelopes keep their timestamp in data, in UNIX seconds.
  return Number.isFinite(event.data?.timestamp) ? event.data.timestamp * 1000 : null;
}
...
function connect() {
  ...
  ws.onmessage = (event) => {
    ...
    // Snapshot items and live fills use the same envelope.
    const fills = msg.type === "snapshot" ? msg.events : msg.type === "event" ? [msg] : [];
    for (const fill of fills) {
      const timestampMs = fillTimestampMs(fill);
      if (timestampMs) lastEventTs = Math.max(lastEventTs || 0, timestampMs);
      // Process fill.data here. Deduplicate by tx_hash + order_hash if needed.
    }
  };
