...
| `games` | array | Full Game objects for every game where `scores.is_live = true` at snapshot time. Same shape as [List Live Games](/api-reference/books/games-live#game-object). |
...
- **Market snapshots are not included** even on `/ws/odds`. For initial market state, call the REST endpoint [`GET /v1/games/{id}/markets`](/api-reference/books/game-markets) once per game you care about, then stream `price_change` deltas from there. Blasting full market state for every live game × every book × every outcome on connect would be tens of MB and pointless for most consumers.
