...
<ParamField body="since" type="number">
  UNIX timestamp in milliseconds. When set, the initial snapshot returns all events **after** this timestamp (up to your tier's snapshot limit) instead of just the most recent N events. Useful for gap-filling after a reconnect or cold start.
  ```json
  "since": 1774412600000
  ```
  The server keeps a rolling window of recent events in memory. At typical throughput, this covers the last 1-3 minutes. Events older than the window are not available via `since`.
</ParamField>
