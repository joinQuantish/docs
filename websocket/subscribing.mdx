...
<ResponseField name="global" type="string">
  ...
  Unfiltered stream of every event (**firehose**, full data stream), including oracle events. High throughput — use with caution. This counts toward your plan's firehose connection limit. To include `position_split`/`position_merge`, add them explicitly via the `event_types` filter override.
</ResponseField>
...
<ParamField body="since" type="number">
  ...
  "since": 1774412600000
  ```
  The server keeps a rolling window of recent events in memory. At typical throughput, this covers the last 1-3 minutes. Events older than the window are not available via `since`. Max lookback varies by tier (free: 30s, starter: 2min, growth/enterprise: 5min).
</ParamField>

<Info>
  **`since` vs `snapshot_count`** — these are two separate snapshot modes. Use `snapshot_count` when you just want the last N events. Use `since` when you need every event after a specific timestamp (e.g. reconnecting after a drop). If you pass both, `since` takes priority and `snapshot_count` is ignored.
</Info>
