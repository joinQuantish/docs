...
The initial snapshot wraps recent events in `events[]`; live messages use the single-event envelope above. Consumers should handle both:

```javascript
function handleMessage(raw) {
  const message = JSON.parse(raw);
  for (const event of message.events ?? [message]) {
    if (event.type === "combo_execution") {
      console.log(event.data.tx_hash);
    }
  }
}
```
...
<ResponseField name="fills" type="string">
  ...
  The initial snapshot uses the same flat fill envelope as live delivery:
  `{"type":"snapshot","count":N,"events":[{"type":"event","data":{...}}]}`.
  For `fills`, `count` is the number of flat fills, not the number of settlement
  transactions. If no fill matches, replay still completes explicitly
  with `{"type":"snapshot","count":0,"events":[]}`.
...
<ResponseField name="combos" type="string">
  ...
  Polymarket combo executions and confirmations. Pending `combo_execution`
  events expose taker-level execution fields plus one maker-perspective row per
  signed order in `trades[]`. Confirmed `combo_status_update` events include
  receipt-derived fills, transfers, fees, gas fields, and confirmation latency
  when the pending event was seen first.

  Each `legs[]` entry includes its decoded structural identifiers. Its nested
  `market` title, slug, image, outcomes, and token IDs are added from the
  in-memory metadata cache when available; the hot path never waits on an
  external lookup.
...
<ResponseField name="redemptions" type="string">
  ...
  Comprehensive payout-redemption feed. It includes direct Conditional Tokens redemptions and Binary, NegRisk, and generic combo AutoRedeemer payouts normalized into the same `redemption` shape. AutoRedeemer variants are distinguished by `data.source`; use a `combos` subscription filtered to `combo_lifecycle` + `action: "Redeem"` when you need combo redemptions only.
</ResponseField>
...
<Warning>
**Tracking a specific wallet in ordinary `settlement` or `trade` events?** Match
by `maker`, not by `taker`.
...
For `type: "combos"`, the `wallets` subscription filter deliberately matches
any involved maker **or** taker. Inside `combo_execution.trades[]`, each row is
still expressed from that signed order maker's perspective.
</Warning>

### Trade filtering

<ParamField body="side" type="string">
  Filter by side. Case-insensitive.

  - Ordinary `settlement` and `trade` events: `"BUY"` or `"SELL"`
  - Combo execution and lifecycle events: `"YES"` or `"NO"`

  For a combo requester's `BUY` or `SELL` direction, use the combo-specific
  `direction` filter instead.
</ParamField>
...
<ParamField body="status" type="string" default="all">
  Filter by chain stage across event families.
  ...
  For combos, `"confirmed"` means a receipt-backed `combo_status_update`. It
  includes both successful and reverted receipts; inspect that event's
  `execution_status` for `CONFIRMED` versus `FAILED`.
</ParamField>
...
<ParamField body="snapshot_count" type="number" default="20">
  ...
  For a `fills` subscription, this counts flat fill events. For example,
  `snapshot_count: 20` returns at most 20 items in `events[]`, each in the exact
  same `{"type":"event","data":{...}}` shape as a live fill.
</ParamField>
...
## Acknowledgement and initial replay

Snapshot-capable event subscriptions acknowledge first, then send replay.
For `fills`, you receive these two messages in this order:

**1. Confirmation:** your subscription ID and any warnings:
```json
{
  "type": "subscribed",
...
**2. Snapshot:** recent flat fills matching your filters:
```json
{
  "type": "snapshot",
  "count": 20,
  "events": [
    { "type": "event", "data": { "timestamp": 1775731431 } }
  ]
}
```
