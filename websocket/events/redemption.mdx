...
<ResponseField name="data" type="object" required>
  <Expandable title="data fields">
    ...
    <ResponseField name="source" type="string">
      Decoder path for bridged redemptions. `AutoRedeemer.BinaryRedemption` and `AutoRedeemer.NegRiskRedemption` identify normal Binary/NegRisk AutoRedeemer payouts; `AutoRedeemer.Redemption` identifies the generic combo AutoRedeemer route. Direct Conditional Tokens redemptions do not use an AutoRedeemer source.
    </ResponseField>
...
To receive combo redemptions only, subscribe to `type: "combos"` with `event_types: ["combo_lifecycle"]` and `action: "Redeem"`. If you subscribe to both surfaces, a generic combo AutoRedeemer payout can appear in both shapes; correlate or deduplicate it with `tx_hash` and `log_index`.
