...
**Pending settlements arrive ~2 seconds before on-chain confirmation.** This is the data that makes PolyNode unique.
...
| Latency | ~0ms from detection | ~2s after detection |

<Tip>
Subscribe with `"status": "pending"` to get the ~2 second edge. You'll receive a separate `status_update` event when the same transaction confirms on-chain, including `latency_ms`.
</Tip>

## Use cases

- **Copy trading** — detect whale trades 2 seconds before confirmation and execute your own order
