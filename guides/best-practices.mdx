...
PolyNode detects Polymarket settlements in the mempool **before** they confirm on-chain. This means every settlement goes through two stages, delivered as **two separate WebSocket events**:

1. **`settlement`** — the trade is detected in the mempool (`status: "pending"`)
