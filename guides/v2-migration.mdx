...
1. **Upgrade your SDK to the V2-ready version.** Older versions won't generate the correct V2 payload.
   - Rust: `polynode >= 0.9.0`. Install with `cargo add polynode@0.9.0`.
   - TypeScript: `polynode-sdk >= 0.8.0`. Install with `npm install polynode-sdk@^0.8.0`.
   - Python: `polynode >= 0.8.0`. Install with `pip install "polynode>=0.8.0"`.
...
5. **Test against V2 now.** Point your SDK at V2 (step 2) and place a small order against `clob-v2.polymarket.com`. The SDK's `ensure_ready` / `ensureReady` wraps any USDC.e into PolyUSD automatically on first use.
6. **Verify V2 confirmation.** Your settlement stream tags V2 trades with `"exchange_version": "v2"` on every event.
...
Your wallet has USDC.e but not PolyUSD. V2 uses PolyUSD as the collateral token. Call the SDK's `ensure_ready` / `ensureReady`. It wraps your USDC.e into PolyUSD automatically and sets V2 spender allowances.
