...
```toml
[dependencies]
polynode = { version = "0.5", features = ["trading", "cache"] }
...
println!("uptime: {}s, events: {}", status.uptime_seconds, status.total_events);
...
println!("positions: {}, profit: {}", profile.positions_count, profile.profit);
...
<Note>Requires the `trading` feature flag: `polynode = { version = "0.5", features = ["trading"] }`</Note>
...
```rust,no_run
use polynode::trading::TradingSigner;
...
<Note>Requires the `cache` feature flag: `polynode = { version = "0.5", features = ["cache"] }`</Note>
...
Helpers for integration tests that need stable public sample wallet addresses:
...
#[tokio::main]
async fn main() {
    // Get a single stable public sample wallet
    let wallet = testing::get_active_test_wallet(false).await;
    println!("test wallet: {}", wallet);

    // Get multiple
    let wallets = testing::get_active_test_wallets(3, false).await;
    for w in &wallets {
        println!("  {}", w);
    }
}
...
The `fresh` boolean remains for source compatibility but is ignored in current
releases. These helpers make no network request; use your own fixture wallets
when a test requires state that your application controls.
