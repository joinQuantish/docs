...
**If you only read data (positions, trades, /resolve):** the response shape is unchanged, with one addition. `/resolve` still returns `safe`, `eoa`, `username`, and `type` exactly as before (`safe` means the preferred Polymarket trading wallet — the deposit wallet when one is deployed), and now also includes a `wallets` object listing every wallet the user has deployed, by type.
