...
|--|------|-------------------|------------------|----------------------|
| **REST API limit** | 60 req/min (1 QPS) | 600 req/min (10 QPS) | 3,000 req/min (50 QPS) | Custom |
...
<Info>
  Some data-heavy endpoints have endpoint-specific limits or paid-tier requirements in addition to your plan's REST API limit. See [Data endpoint limits](#data-endpoint-limits) below for details.
</Info>
...
The enriched analytics endpoints count against your plan's REST API limit. Expensive endpoints may also have their own endpoint-specific caps called out in their endpoint reference.

Enriched endpoints include:
