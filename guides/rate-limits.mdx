...
The wallet and market data endpoints (positions, trades, P&L) have a separate shared throughput limit of **10 requests per second** across all users. This applies to:
...
Batch requests (multiple wallets) count each wallet as one request toward this limit. For example, a 5-wallet batch counts as 5 requests. If the limit is exceeded, you'll receive a `429` response. Wait briefly and retry.

<Tip>
  This limit is independent of your per-key rate limit. Even with a high-tier plan, these endpoints share the same throughput cap. For real-time data, use the [WebSocket stream](/websocket/overview) instead.
</Tip>
