...
description: "Subscribe to 30-second and 60-second Chainlink TWAP prices, or explicitly select the spot stream."
icon: "wave-square"
---
...
|---|---|
| **August 4, 2026** | Production Chainlink 30-second and 60-second TWAP streams became available. Select them explicitly with `twap_windows`. |
| **August 7, 2026 at 00:00 UTC** | Affected markets switched to Chainlink TWAP resolution. At the same time, polynode's default `chainlink` subscription switched from spot prices to both TWAP windows. |
...
The default `chainlink` subscription follows Main and now delivers both TWAP
windows. To receive the spot stream instead, select it explicitly:
...
`price_source: "spot"` with `twap_windows` in one subscription.

The subscription acknowledgement confirms the explicit selection:
...
<Note>
  Spot and TWAP subscriptions start with the next update and have no snapshot,
  history, or replay.
</Note>
...
It now delivers both 30-second and 60-second TWAP events. The WebSocket URL,
API key, subscription type, and existing JSON fields did not change. Clients
that still need spot can use the explicit `price_source: "spot"` selection
above.

If your strategy uses one market duration, add the matching `twap_windows`
filter so your code receives only that window.
