...
<Info>
This is a PolyNode-run, point-in-time network study. Historical latency rows use retained PolyNode measurements; today's sensor race ran against an isolated staging sink and made no production changes. A benchmark-only external Alchemy RPC supplied immutable block headers, receipts, and logs. It supplied no pending events or arrival timestamps used in the latency measurements.
</Info>
...
<CardGroup cols={2}>
  ...
  <Card title="Go led Rust by 70 ms" icon="stopwatch">
    Today's controlled sensor race did not support a language-regression explanation.
  </Card>
</CardGroup>
...
| August 4 control: isolated Go first-seen | 1.5 s | 4,780 | 2.137 s | 2.866 s | 1.425 / 1.911 |

The August row is a different, sensor-level clock and is shown as a control rather than folded into the before-and-after calculation.
...
There is a tradeoff. PIP-75 notes that shorter intervals leave less time for each block to propagate. Validator clocks, peer connectivity, and IO therefore matter more as cadence falls. Polygon chose 1.5 seconds as an operating point above the new one-second consensus floor, not as an arbitrary timestamp edit.
...
## Today's sensor control

We also tested the original alternative explanation: perhaps moving between Rust and Go sensors had cost a second.

On August 4, current Go and Rust sensors ran with the same 200-peer cap, the same host clock, and the same isolated local sink. Across 1,918 transaction hashes seen by both, Go detected the transaction first by **70.263 milliseconds at the median**, 321.064 milliseconds at P90, and 1.272 seconds at P99. Median sensor-to-sink overhead was about one millisecond.

The current Go result was 2.137 seconds early at the median and 2.866 seconds at P90 over 4,780 included transactions after excluding clearly stale peer replays. The current Rust result was 2.034 seconds and 2.792 seconds over 1,179 included transactions. Those samples do not support a Go decoding regression as the missing second.

We also replayed the archived April Rust binary in isolation. It continued receiving blocks but detected no current V2 settlement transactions because its custom path handles full transaction broadcasts and not the hash-announcement/fetch path now carrying useful bodies. Replacing the current fleet with that binary would sacrifice coverage, not recover historical timing.

## Can a four-second lead come back?

Not consistently through a language swap or faster ABI decoding. Four seconds on a 1.5-second chain is 2.67 block intervals. The historical P90 was about 1.97 block intervals, and the July P90 is still about 1.97.

Recovering a durable four-second P90 now would require learning about transactions roughly one full second earlier in Polygon's propagation path than the position that produced the old 3.95-second P90. That points upstream: closer peering to transaction origin, validator-adjacent topology, geographic diversity, and per-peer first-arrival instrumentation. Those are worthwhile directions, but they are different from restoring a lost decoder optimization.
...
We located one directly relevant independent observation: a [May 14 post by Ryan Chern](https://x.com/ryanchern/status/2055006540101546020) connected the first 2.0-to-1.75-second cut to a **2.40-second median** Polymarket trade-preconfirmation gap.

We did not locate a published study that followed both cadence cuts, normalized the before-and-after Polymarket window by block interval, and controlled the sensor implementation. Public-search results are not a complete denominator, so this is a description of the material we found, not a claim that nobody else has measured it.
...
- April and July use PolyNode's public `detected_at` against Polygon's block header timestamp. The August control uses the sensor's `time_first_seen`. It is intentionally reported separately.
- Polygon header timestamps have one-second resolution, while detections use a host clock. Quantiles over hundreds or thousands of transactions reduce the visible quantization, but the comparison is not a single-clock nanosecond benchmark.
- The historical rows are retained aggregate measurements. Raw August sensor arrivals and binary hashes are preserved internally; raw mempool payloads are not part of the public bundle.
...
- All results are date-, implementation-, and vantage-point-specific. Polygon can tune block time again under the mechanism enabled by PIP-75.
...
Last evidence audit: **2026-08-04 04:16 UTC**.
