...
<CardGroup cols={2}>
  ...
  <Card title="3.86 second median lead" icon="bolt">
    PolyNode's fill stream arrived first for 98.995% of exact fill pairs.
  </Card>
...
PolyNode [`status_update`](/websocket/events/status-update) is the transaction-level confirmation comparison requested for Struct confirmed. PolyNode [`trade`](/websocket/events/trade) is the stricter fill-level comparison. PolyNode [`settlement`](/websocket/events/settlement) with `status=pending` is the pre-confirmation path.
...
| PolyNode `status_update` vs Struct confirmed | 18,041 | +3.963 s | +5.083 s | 100.00% |
| PolyNode confirmed fill vs Struct confirmed record | 48,281 | +3.862 s | +5.061 s | 99.00% |
| PolyNode pending vs RTDS activity | 17,777 | +1.984 s | +2.858 s | 100.00% |
| PolyNode `status_update` vs RTDS activity | 17,777 | -0.171 s | +0.428 s | 34.48% |
