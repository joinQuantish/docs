...
## 2026-05-02 — SDK v0.13.3 (Rust) / v0.10.6 (TS) / v0.10.3 (Python) — deposit wallet onboarding

`ensureReady()` now handles the full onboarding flow for deposit wallet users: derives the wallet address, deploys via `WALLET-CREATE` if needed, and sets all V2 approvals via a signed `WALLET` batch. Same one-call pattern as Safe wallets.

```bash
cargo add polynode@0.13.3
npm install polynode-sdk@0.10.6
pip install polynode==0.10.3
```

---

## 2026-05-02 — SDK v0.13.1 (Rust) / v0.10.5 (TS) / v0.10.2 (Python) — POLY_1271 order signing

Deposit wallet users can now place V2 orders. When `signatureType` is `POLY_1271` (3), the SDK automatically wraps the EIP-712 signature using the ERC-7739 TypedDataSign format required by Polymarket's deposit wallet contracts. No additional configuration needed — `signV2Order` (TS) / `create_signed_order_v2` (Python) / `order()` (Rust) detect the signature type and handle wrapping transparently.

```bash
cargo add polynode@0.13.1
npm install polynode-sdk@0.10.5
pip install polynode==0.10.2
```

---

## 2026-05-02 — SDK v0.13.0 (Rust) / v0.10.4 (TS) / v0.10.0 (Python)

All three SDKs now support Polymarket's new deposit wallet type:

- **New signature type:** `SignatureType::Poly1271` (Rust) / `SignatureType.POLY_1271` (TS/Python) — value `3`
- **New derivation:** `deriveDepositWalletAddress(eoa)` (TS) / `derive_deposit_wallet_address(eoa)` (Rust/Python) computes the deterministic deposit wallet address for any EOA
- **Auto-detection:** `detectWalletType()` and `ensureReady()` automatically detect whether an EOA has a Safe, proxy, or deposit wallet deployed

Fully backward compatible. Existing Safe and proxy wallet flows are unchanged. Install the latest version to get deposit wallet support.

```bash
# Rust
cargo add polynode@0.13

# TypeScript
npm install polynode-sdk@0.10.4

# Python
pip install polynode==0.10.0
```

---

## 2026-05-02 — Deposit wallet support on `/resolve`

The [Resolve Wallet](/api-reference/wallets/resolve) endpoint now supports Polymarket's new deposit wallet type alongside the existing Safe proxy wallets. Both forward resolution (EOA to deposit wallet) and reverse resolution (deposit wallet to EOA) work automatically.

**New `type` field** in all responses:
...
Possible values: `"safe"` (Gnosis Safe proxy), `"deposit_wallet"` (new deposit wallet), or `"proxy"` (legacy proxy).

**Backward compatible.** The `safe` field still contains the trading wallet address regardless of type. Existing integrations continue working without changes. The `type` field is additive.
...
The server walks the full history with parallel time-bucketed queries internally. Optional `from` and `to` query parameters narrow the window.

**Limits:**

- Hard cap **250,000 trades** per call. Beyond that, response includes `"partial": true` with `"partial_reason": "hard_cap_250000"`.
- Wall-clock budget **180 seconds**. First cold-cache calls on busy markets/heavy wallets may take **1-3 minutes**. Subsequent calls hit Redis cache and return in **<1 second**.
...
**Tier requirement:** **Growth plan ($200/mo) or above.** Starter and free tier requests receive `403`. The bulk endpoints are gated to paid plans because of the heavy server-side compute (one call drives 100s of internal sub-queries) and large payloads.
...
Updated upstream timeout to 180 seconds across the gateway path so cold-cache calls complete cleanly.
