...
Two new v2 endpoints return complete position history with accurate P&L sourced from onchain settlement data. Every `realized_pnl` value has been verified to match Polymarket's own numbers to the penny.
...
| `positions[].total_bought` | Total USDC cost basis |
...
```bash
curl "https://api.polynode.dev/v2/wallets/0xbddf.../closed-positions?limit=3&sortBy=REALIZEDPNL&sortDirection=DESC" \
  -H "x-api-key: YOUR_KEY"
```
...
The TypeScript and Rust SDKs now fetch onchain position data automatically during backfill. No code changes needed if you're already using `cache.start()`. The backfill makes one additional API call per wallet (the onchain positions endpoint above) and stores the results in your local SQLite database.
...
```typescript
const dash = cache.walletDashboard("0xbddf...");
// dash.realized_pnl     → 17183579.48
// dash.pnl_confidence   → "full"
// dash.token_pnl        → per-token P&L breakdown (446 entries)
// dash.total_pnl        → unrealized P&L on open positions
```
