...
## 2026-04-30 — TypeScript SDK 0.9.6
...
- **Exported interface** — `TradingStorage` and `InMemoryStorage` are exported so you can build custom adapters (e.g. `bun:sqlite`, Postgres, Redis).

```typescript
// Bun users — no better-sqlite3 needed:
...
Upgrade:
```bash
npm install polynode-sdk@0.9.6
```
