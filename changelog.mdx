...
## 2026-05-02 — SDK v0.13.0 (Rust) / v0.10.1 (TS) / v0.10.0 (Python)

All three SDKs now support Polymarket's new deposit wallet type:

- **New signature type:** `SignatureType::Poly1271` (Rust) / `POLY_1271` (TS/Python) — value `3`
- **New derivation:** `derive_deposit_wallet_address(eoa)` computes the deterministic deposit wallet address for any EOA
- **Auto-detection:** `detect_wallet_type()` and `ensureReady()` automatically detect whether an EOA has a Safe, proxy, or deposit wallet deployed
