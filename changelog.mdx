...
## 2026-04-14 — Sports Scores WebSocket

### New: Real-time Sports & Esports Scores

A dedicated WebSocket endpoint for live game state across every sport polynode covers — tennis, basketball, soccer, hockey, baseball, football, MMA, cricket, esports, and more. One connection, all sports, server-side filtering.
...
ws.on("open", () => {
  ws.send(JSON.stringify({
    action: "subscribe",
    type: "sports_scores",
    filters: { sport: ["tennis", "basketball"], status: ["inprogress"] }
  }));
});
...
ws.on("message", (raw) => {
  ...
  if (msg.type === "score_update") {
    const g = msg.data;
    console.log(`${g.awayTeam} ${g.score} @ ${g.homeTeam} (${g.period})`);
  }
});
```

**Highlights:**
- **One unified stream** — all sports on a single connection
- **Server-side filtering** — by sport, league, game ID, status, or tournament
- **Snapshot on connect** — receive the current state of every live game immediately
- **Multiple subscriptions per connection** — open one socket, register many filtered views
- **Auto-reconnect friendly** — snapshot replay catches you up automatically after a disconnect
- **REST snapshot helper** — `GET https://sports.polynode.dev/games` returns the current cache for browsing before you subscribe
- **Available on Starter tier and above**

[See WebSocket → Sports Scores → Overview](/websocket/sports/overview)
