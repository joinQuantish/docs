...
### API — Enriched Data Endpoints (v2)

Eight new REST endpoints for trader analytics, market discovery, and platform trends.

- `GET /v2/leaderboard` — Top 20 traders ranked by profit or volume (daily, weekly, monthly, all-time)
- `GET /v2/trader/{wallet}` — Full trader profile: PnL, volume, trade count, largest win, portfolio value
- `GET /v2/trader/{wallet}/pnl` — PnL time series at 4 resolutions (1D, 1W, 1M, ALL)
- `GET /v2/trending` — Carousel highlights, breaking markets, hot topics, featured events, biggest movers
- `GET /v2/activity` — Platform-wide live trade feed (last 50 trades with tx hashes)
- `GET /v2/event/{slug}` — Full event data with all sub-markets, outcome prices, condition IDs
- `GET /v2/movers` — Markets with the largest 24h price changes
