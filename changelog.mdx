...
Newly-added wallets are scored within ~30s (on-add freshening). The full pool refreshes daily, processed in chunks of 50 wallets spread across the day to keep peak load bounded. Each result row includes `computed_at` so you can render freshness in your UI.
...
Internal: switched the underlying Goldsky queries from a 2-query maker/taker split to a single `or:` operator query with timestamp-cursor pagination. Halves Goldsky load per request.
