...
## 2026-05-27 - V3 wallet P&L alignment, fees, rebates, and rewards

Updated V3 wallet accounting surfaces.

What's new:

- Default/all-time `GET /v3/wallets/{address}/pnl/events` now returns a single summary-backed bucket from the wallet summary source so it agrees with `GET /v3/wallets/{address}/pnl`.
- Explicit time-windowed P&L events (`period`, `after`, or `before`) still use the indexed `pnl_event` stream and ignore timestamp-0 artifacts.
- Added `GET /v3/wallets/{address}/fees-paid` for fee-bearing `OrderFilled` rows.
- Added `GET /v3/wallets/{address}/rebates` as a Polymarket public maker-rebate wrapper.
- Added `GET /v3/wallets/{address}/rewards` as a caller-authenticated Polymarket LP reward earnings proxy.
- Added public reward-market config wrappers at `GET /v3/rewards/markets` and `GET /v3/rewards/markets/{condition_id}`.

Notes:

- Fees paid, maker rebates, and LP rewards are intentionally separate surfaces. They should not be merged into headline P&L unless the response explicitly labels the category and source.
- Wallet activity endpoints still cover conditional-token activity, not PolyUSD ERC-20 deposits or withdrawals.
...
Indexed scan over `pnl_event` — most full-history queries return sub-second.
