...
## 2026-05-10 — Sort trades by order hash

`/v2/onchain/trades` now supports `sort_by=order_hash`. All fills from the same limit order are grouped together in the response, sorted chronologically within each order. Orders are ranked by their most recent fill.

Every fill is still returned individually. No data is collapsed or aggregated.
...
Useful for tracking how a single limit order was filled across multiple transactions over time.
