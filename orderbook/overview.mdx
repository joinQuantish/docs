...
<Info>
  Successfully delivered Order Book payloads and Chainlink price payloads share the **Order Book + Chainlink** transfer meter in your [usage dashboard](/usage/overview). Compression reduces counted transfer because the meter uses the encoded payload delivered to the connection. Heartbeats, acknowledgements, control frames, and failed sends do not count.
</Info>
