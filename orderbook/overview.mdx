...
With opt-in local-book integrity:
```
wss://ob.polynode.dev/ws?key=pn_live_YOUR_KEY&integrity=1
```

Compression and integrity can be combined. See [Orderbook Integrity (PN1)](/orderbook/integrity) for the checksum algorithm and copyable Node.js, Python, Go, and Rust references.
