...
Subscribe to orderbook updates for specific markets. You can identify markets by **slug**, **condition ID**, or **token ID** — the server resolves them all to token IDs transparently.
...
| `markets` | string[] | Array of market identifiers. Each can be a slug (e.g. `"bitcoin-100k"`), condition ID (e.g. `"0x561ff..."`), or CLOB token ID (numeric string). Slugs and condition IDs are resolved to their corresponding token IDs automatically. |
...
Acknowledgment after a successful subscribe. Includes a breakdown of how identifiers were resolved, so you can verify your slugs/condition IDs matched.
