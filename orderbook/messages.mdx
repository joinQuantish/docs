...
{
  "action": "resync",
  "markets": ["73624432805780182150964443951045800666977811185963019133914618974858599458273"]
}
```

This action is available on connections opened with `integrity=1`. Every requested market must already be subscribed. Omit `markets` or pass an empty array to resync the full current subscription. A successful `resyncing` acknowledgement always leads to a new ordered anchor and a matching completion marker. If an anchor for any requested token is already in flight, the server returns `resync_in_progress`; keep that token gated and retry after the supplied delay.
...
<Tabs>
  ...
  <Tab title="PN1 integrity">
    ...
    The acknowledgement is queued before the standalone PN1 anchor snapshots. See [Orderbook Integrity](/orderbook/integrity).
  </Tab>
</Tabs>
...
| `integrity` | object | Present in PN1 mode. A full snapshot has `protocol`, `anchor: true`, `seq`, and the post-snapshot `checksum`. |
...
| `asset_id` | string | PN1 only: repeats the token ID at the event level |
| `integrity` | object | PN1 depth frames: `protocol`, `prev_seq`, `seq`, and post-update `checksum` |
...
| `integrity` | object | PN1 only: `protocol`, `prev_seq`, `seq`, and the post-update `checksum` |
...
| `invalid_integrity_option` | The `integrity` URL parameter was present but was not `0` or `1`. |
| `integrity_unavailable` | PN1 is temporarily disabled by the service operator. Retry later or reconnect without `integrity=1`. |
| `integrity_firehose_unsupported` | PN1 was requested with `markets:["*"]`; use an explicit list of up to 500 resolved tokens. |
| `integrity_capacity` | The process-wide 5,000-unique-token PN1 mirror limit is full. The previous valid subscription is preserved. |
| `resync_resolved_empty` | A non-empty PN1 resync request did not resolve to any subscribed token. The current subscription is unchanged. |
| `resync_not_subscribed` | A PN1 resync requested a token outside the current subscription. |
| `resync_in_progress` | An anchor is already in flight for at least one requested PN1 token. Keep the token gated and retry after `retry_after_ms`. |
| `integrity_backpressure` | Gapless delivery can no longer be guaranteed. The socket closes; reconnect for fresh anchors. |
| `integrity_internal_reset` | PN1 state was reset or an invariant failed. The socket closes; reconnect for fresh anchors. |
