...
```bash
curl "https://api.polynode.dev/v2/sports/games/nba-cle-nyk-2026-05-31/context?sources=x,ddg_instant&query_set=injuries&max_queries=2&max_per_query=5&include_state=true&price_limit_tokens=20" \
  -H "x-api-key: YOUR_KEY"
```
...
```json
{
  ...
  "sources_requested": ["x", "ddg_instant"],
  "sources": {
    "x": {
      "status": "ok",
      "result_count": 5,
    ...
    "ddg_instant": {
      "status": "ok",
      "note": "DuckDuckGo Instant Answer is not a full search-results API; live sports queries often return no instant answer.",
      "queries": []
    }
  },
  "state": {
...
| --- | --- |
| `sources` | Comma-separated sources. Defaults to `x`. Supported: `x`, `ddg_instant`. `ddg` and `web` alias to `ddg_instant`. |
...
The `x` source uses the same X Search beta quota and 1 request/second key-level rate limit as `/v2/x/search`. Each generated X query consumes one X Search quota unit. The `ddg_instant` source is DuckDuckGo Instant Answer, not a full search results API, so live sports queries may return no external results.
