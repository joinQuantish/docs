...
`order_hash` identifies the order owned by that row's `maker`. Therefore, when
you are reconciling an order submitted for a wallet:
...
2. Keep only rows with `side: "maker"` and a matching `maker` address. A
   `side: "taker"` row describes an order owned by the counterparty, so its
   `order_hash` is not the queried wallet's submitted order.
...
4. Group matching rows by `order_hash`. The endpoint returns one row per fill,
   and a partially filled order can appear in several rows with the same hash.
...
improvement may make the received leg larger than its preview minimum. Also
require the expected token, derived BUY/SELL direction, maker address, and a
narrow submission-time window. If exactly one hash group passes every
condition, retain that hash as the durable correlation key. If none or more
than one pass, the result is still ambiguous: mark it for operator/user review
and never automatically resubmit the consumed order.
