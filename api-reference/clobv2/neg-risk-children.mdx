...
Drill into a single [neg-risk event](/api-reference/clobv2/neg-risk-events) to get every child market (one per candidate) with its own `condition_id`, `question` (formatted `"Will {candidate} win?"`), `slug`, `active`/`closed` state, lifetime volume, and liquidity.
...
| `children[].liquidity` | string (numeric) | Current AMM liquidity in USDC (relevant only for v1 FPMM-backed markets; `"0"` on pure v2 markets). |
