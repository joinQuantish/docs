...
| `unrealized_pnl` | number | Unrealized profit/loss in USD. For resolved markets, calculated using the final outcome price ($1 for winners, $0 for losers). For open markets, based on the current market price (refreshed every 15 minutes). Returns `0` for closed positions. |
...
- The `unrealized_pnl` field shows the paper profit/loss on the remaining open shares. For resolved markets, this uses the final settlement price ($1 for winners, $0 for losers). For active markets, the current market price is used (refreshed every 15 minutes).
