...
| `initial_value` | number \| undefined | Cost basis in USD of the currently held shares (`size × cost_per_share`). The exact value Polymarket uses internally to compute `cashPnl`. Use this if you need the basis number that matches what users see in the Polymarket UI. |
