...
| `sort_by` | string | No | Set to `order_hash` to group fills by order. All fills from the same limit order appear adjacent, sorted by time within each group. Groups ordered by most recent fill. Default sort is by timestamp. |
