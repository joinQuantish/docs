...
| `side` | string | Exchange role: `"maker"` or `"taker"`. Only present when filtering by `wallet`. |
| `direction` | string | Trader intent: `"BUY"` or `"SELL"` from the queried wallet's perspective. `BUY` when the wallet contributed USDC and received outcome tokens; `SELL` when the wallet contributed outcome tokens and received USDC. Only present when filtering by `wallet` (no anchor wallet ⇒ no direction). Independent axis from `side` — they answer different questions and both are populated together when applicable. |
