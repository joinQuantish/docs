...
<Note>
`total_realized_pnl` is a raw sum of all individual position `realized_pnl` values. This number will differ from the aggregate PnL shown on a Polymarket profile page, which applies its own netting and aggregation logic. For Polymarket's own aggregate PnL, use the [trader profile](/api-reference/enriched/trader-profile) endpoint's `totalPnl` field.
</Note>
