...
| `positions[].total_bought` | number | Total USDC spent buying this token |
...
- `avg_price` = 0.40
- `total_bought` = $10,000 (cost basis)
- `realized_pnl` = $10,000 × (1.0 - 0.40) / 0.40 = $15,000

These numbers match the values shown on the Polymarket portfolio page to the penny.

<Note>
`total_realized_pnl` is the sum of all individual position P&L values. This represents total realized gains and losses. It does not include unrealized P&L on open positions. For a combined realized + unrealized total, use the [trader profile](/api-reference/enriched/trader-profile) endpoint's `totalPnl` field.
</Note>
