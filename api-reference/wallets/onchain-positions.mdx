...
The simplest discovery path is to fetch a wallet's positions without a filter, then read the union of `tag_slugs` across the response — that's the tag namespace the wallet has actually traded in. For a global tag list, scan a few high-volume markets via `/v2/onchain/positions` and collect distinct `tag_slugs`.
