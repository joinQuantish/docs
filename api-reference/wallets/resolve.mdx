...
Look up the full identity for any Polymarket user: preferred trading wallet, EOA (externally owned account), username, wallet type, and every Polymarket wallet they have deployed. Accepts any identifier as input and returns all fields.
...
One user can have more than one wallet — for example an old Safe from years ago plus a new deposit wallet. The `wallets` object in every response lists everything they have deployed.
...
<ResponseField name="type" type="string">
  Wallet type for the address returned in `safe`. One of `"deposit_wallet"`, `"safe"`, or `"proxy"` — see [wallet types](#understanding-polymarket-wallet-types) above.
</ResponseField>
...
- **Multi-wallet users** — use the `wallets` object to connect a user's old Safe history with their new deposit wallet activity
