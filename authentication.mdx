...
All active API keys owned by the same account contribute to one shared monthly usage total. Creating, renaming, rotating, or using multiple keys does not create separate allowances or reset usage.

The [usage dashboard](https://polynode.dev/dashboard/usage) is authenticated by your signed-in account; you do not submit a raw API key to view it. See [Monthly usage allowances](/guides/rate-limits#monthly-usage-allowances) for the exact meters and included plan capacity.
