...
- Wall-clock budget **180 seconds**. First calls on busy markets or heavy wallets may take **1-3 minutes**. Repeated calls for the same scope and window typically return in **under 1 second**.
- Typical payload **1-50 MB**. Worst case (capped): **100-200 MB JSON**.
- Results are reused for **5 minutes** per `(scope, from, to)` combination. Partial results are **not reused**.
