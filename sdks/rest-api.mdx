...
<Tabs>
  <Tab title="TypeScript">
    ...
    console.log(markets.count, results.results.length);
    ```
  </Tab>
  <Tab title="Python">
    ```python
    ...
    with PolyNode(api_key=os.environ["POLYNODE_API_KEY"]) as pn:
        ...
        print(markets.count, len(results.results))
    ```
  </Tab>
  <Tab title="Rust">
    ```rust
...
| Existing code already using `market()`, `search()`, or the equivalent language method | Keep the top-level method |
