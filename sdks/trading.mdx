...
```bash
npm install polynode-sdk@0.7.3 viem better-sqlite3 @polymarket/clob-client @polymarket/builder-relayer-client @polymarket/builder-signing-sdk
```
...
Before placing orders on V2 markets, you need to wrap your USDC.e into PolyUSD. Wrap amounts are in raw 6-decimal units (`1_000_000n` = $1):

```typescript
// Wrap 100 USDC.e → PolyUSD
await trader.wrapToPolyUsd(100_000_000n);

// Check PolyUSD balance (returns raw bigint, divide by 1e6 for dollars)
const balanceRaw = await trader.getPolyUsdBalance();
console.log(`PolyUSD: $${Number(balanceRaw) / 1e6}`);
```

On V2, `ensureReady()` also refreshes the CLOB's cached balance-allowance view after setting approvals. If you change approvals outside of `ensureReady`, call `await trader.refreshBalanceAllowance()` before placing orders — otherwise the CLOB can reject with `"not enough balance / allowance"` until its cache catches up.
