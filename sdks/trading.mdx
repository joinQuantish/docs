...
For optional zero-builder attribution backed by the signing wallet, see [User-owned execution](/sdks/user-owned-execution). Existing integrations stay on builder mode unless they opt in.
...
```bash Python
python -m pip install "polynode[trading]>=0.13.0,<0.14"
```

```toml Rust
[dependencies]
polynode = { version = "0.16.0", features = ["trading"] }
