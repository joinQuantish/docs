...
Pick a coin and interval to get the current market slug. The `{timestamp}` is the Unix epoch (seconds) of the window start, always aligned to the interval boundary.

| Coin | 5-minute slug | 15-minute slug |
|------|--------------|----------------|
| BTC | `btc-updown-5m-{timestamp}` | `btc-updown-15m-{timestamp}` |
| ETH | `eth-updown-5m-{timestamp}` | `eth-updown-15m-{timestamp}` |
| SOL | `sol-updown-5m-{timestamp}` | `sol-updown-15m-{timestamp}` |
| BNB | `bnb-updown-5m-{timestamp}` | `bnb-updown-15m-{timestamp}` |
| XRP | `xrp-updown-5m-{timestamp}` | `xrp-updown-15m-{timestamp}` |
| DOGE | `doge-updown-5m-{timestamp}` | `doge-updown-15m-{timestamp}` |
| HYPE | `hype-updown-5m-{timestamp}` | `hype-updown-15m-{timestamp}` |

Compute the current window timestamp:
