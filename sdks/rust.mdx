...
```toml
[dependencies]
polynode = { version = "0.5.3", features = ["trading", "cache"] }
...
<Note>Requires the `trading` feature flag: `polynode = { version = "0.5.3", features = ["trading"] }`</Note>
...
<Note>Requires the `cache` feature flag: `polynode = { version = "0.5.3", features = ["cache"] }`</Note>
