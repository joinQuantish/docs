...
```toml
[dependencies]
polynode = { version = "0.5.2", features = ["trading", "cache"] }
...
<Note>Requires the `trading` feature flag: `polynode = { version = "0.5.2", features = ["trading"] }`</Note>
...
<Note>Requires the `cache` feature flag: `polynode = { version = "0.5.2", features = ["cache"] }`</Note>
