...
```toml
[dependencies]
polynode = { version = "0.6.0", features = ["trading", "cache"] }
...
<Note>Requires the `trading` feature flag: `polynode = { version = "0.6.0", features = ["trading"] }`</Note>
...
<Note>Requires the `privy` feature flag: `polynode = { version = "0.6.0", features = ["trading", "privy"] }`</Note>
...
```rust,no_run
use polynode::trading::{TraderConfig, ExchangeVersion};
...
# async fn example(trader: &mut polynode::trading::PolyNodeTrader) -> polynode::Result<()> {
// Wrap USDC.e → PolyUSD
let result = trader.wrap_to_polyusd(100.0).await?;

// Unwrap PolyUSD → USDC.e
let result = trader.unwrap_from_polyusd(50.0).await?;

// Check balances
let polyusd = trader.get_polyusd_balance().await?;
let usdce = trader.get_usdce_balance().await?;
println!("PolyUSD: {}, USDC.e: {}", polyusd, usdce);
# Ok(())
# }
```
...
<Note>Requires the `cache` feature flag: `polynode = { version = "0.6.0", features = ["cache"] }`</Note>
