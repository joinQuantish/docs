...
Python 0.14.0 exposes the complete shared V3 operation registry through both `PolyNode` and `AsyncPolyNode`. V3 JSON decimal fractions are decoded as `Decimal` so money and prices are not silently rounded through binary floating point.
...
print(V3_OPERATION_COUNT)  # 120 in 0.14.0
