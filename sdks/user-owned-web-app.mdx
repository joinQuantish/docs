...
not supported in user-owned mode.
...
1. Load the wallet's authoritative open orders with
   ...
   (Rust).
2. A fully filled order may no longer be open. Query the wallet's
   [V2 fill history](/api-reference/clobv2/wallet-trades) and compare the
   wallet, token, side, canonical amounts/price, and narrow preparation-time
   window. Retain the returned order hash as the durable correlation key.
3. Repeat those reads after a short bounded delay to allow indexing. If one
   exact candidate is found, attach it to the attempt. If zero or multiple
   candidates remain, mark the attempt `needs_reconciliation` and surface it
   for operator/user review.
