...
<Note>
Use TypeScript `polynode-sdk >= 0.14.1`, Python `polynode >= 0.14.0`, or Rust
...
```bash
pip install "polynode[trading]>=0.14.0,<0.15"
```
...
Version 0.14.1 accepts viem's standard `EIP1193Provider` directly; no provider
...
from the consumed state: `requestId`, authenticated user, wallet/funder,
`tokenId`, side, canonical price/size, raw maker/taker amounts, and preparation
time. Set its status to `submitting`. Do not put the signature, credential
bundle, or reusable prepared state in that record.
...
2. A fully filled order may no longer be open. Query
   ...
   `attempt.maker`, which is the signing preview's `order.maker` and the
   session's `funderAddress`. Do **not** query the controlling EOA for a Safe or
   deposit-wallet order merely because it signed the request.
3. In that response, `side` means the wallet's role (`maker` or `taker`), not
   order direction. For the submitted order, consider only `side: "maker"`
   rows whose `maker` equals `attempt.maker`. Derive direction from the raw
   token sides: `maker_token_id == "0"` and `taker_token_id == tokenId` is a
   BUY; the reverse is a SELL. Ignore counterparty-role and compound
   mint/merge rows.
4. Group the remaining rows by `order_hash`. Each row is one fill, so partial
   fills of one order reuse the hash. Convert `maker_usdc` and `taker_usdc`
   from decimal strings to exact raw six-decimal integers before summing; do
   not compare them directly with the preview's raw integer strings or parse
   them as floating-point numbers. The fill-history page gives the exact
   conversion and candidate rules.
5. Repeat the open-order and fill reads after a short bounded delay to allow
   indexing. Attach a hash only when exactly one group matches maker, token,
   derived BUY/SELL direction, the narrow submission-time window, and the
   prepared order's exact amount/limit constraints. If zero or multiple groups
   remain, mark the attempt `needs_reconciliation` and surface it for
...
For the browser-memory model, version 0.14.1 exposes the same authenticated
open-order read as
`await session.getOpenOrders({ assetId: attempt.tokenId })`. Keep the
non-secret attempt record outside the session so it survives a tab refresh;
use the same fill-history comparison and ambiguity rules. The browser session
does not invent an idempotency key that the order API does not support.
