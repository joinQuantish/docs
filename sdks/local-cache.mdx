...
<Steps>
  ...
  <Step title="Start the cache">
    ...
    await cache.start();
    // Backfill runs automatically, live stream starts immediately
    ```
  </Step>
  <Step title="Query locally">
    ```typescript
    // All queries hit the local SQLite DB — zero API calls
...
Backfill fetches the most recent trades for each wallet from the API. Each wallet takes one request.
...
All queries run against the local SQLite database. No API calls.

### Wallet Trades

```typescript
const trades = cache.walletTrades('0xabc...', {
  limit: 100,         // default: 100
  offset: 0,          // for pagination
  side: 'BUY',        // filter by side
  since: 1710000000,  // Unix seconds
  until: 1712000000,
  orderBy: 'timestamp_desc', // or 'timestamp_asc'
});

// Each trade includes:
// tx_hash, timestamp, maker, taker, token_id, condition_id,
// market_title, market_slug, outcome, side, price, size
```

### Wallet Positions

Positions are computed on-the-fly from cached trades. No separate storage needed.
...
// Each position:
// {
//   token_id, condition_id, market_title, market_slug, outcome,
//   net_qty,        // net shares held (BUY - SELL)
//   avg_price,      // volume-weighted average entry price
//   trade_count,    // number of trades
//   first_trade_at, // Unix seconds
//   last_trade_at,  // Unix seconds
// }
```
...
```typescript
const allPositions = cache.multiWalletPositions([
  '0xabc...', '0xdef...', '0x123...'
]);
...
```typescript
const trades = cache.marketTrades('0xcondition...', {
  limit: 100,
  side: 'SELL',
});
```
...
```typescript
const positions = cache.marketPositions('0xcondition...');
```

### Token Trades

```typescript
const trades = cache.tokenTrades('12345...', { limit: 50 });
```

### Trade by Transaction Hash

```typescript
const trades = cache.tradeByTxHash('0xabc123...');
```

### Wallet Settlements

```typescript
const settlements = cache.walletSettlements('0xabc...', { limit: 20 });
```
...
//   trade_count: 4592,
//   settlement_count: 16,
//   db_size_bytes: 13500000,
//   oldest_trade_at: 1729216000,
//   newest_trade_at: 1774113600,
//   backfill_entities: 10,
//   backfill_complete: 10,
//   backfill_pending: 0,
...
Add or remove wallets programmatically:

```typescript
// Add a wallet — backfill starts immediately
cache.addToWatchlist([
  { type: 'wallet', id: '0xnew...', label: 'new-trader' }
]);

// Remove a wallet
cache.removeFromWatchlist([
  { type: 'wallet', id: '0xold...' }
]);
```
...
for (const p of positions) {
  console.log(`${p.outcome}: ${p.net_qty} shares @ ${p.avg_price.toFixed(4)}`);
}
