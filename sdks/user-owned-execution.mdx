...
export async function signAuthorizationMessage(
  message: string,
  browserProvider: Parameters<typeof custom>[0],
) {
  ...
  // Return only the address and signature to your backend.
  return { address, signature };
}
```

### Python browser backend
...
async def begin_for_browser(address: str) -> UserRelayerAuthorizationChallenge:
    challenge = await begin_user_relayer_authorization(
        "https://trade.polynode.dev",
        os.environ["POLYNODE_API_KEY"],
        address,
    )
    # Keep the complete challenge in short-lived backend storage.
    # Return only challenge.message to the browser.
    return challenge

async def finish_from_browser(
    address: str,
    challenge: UserRelayerAuthorizationChallenge,
    signature_returned_by_browser: str,
) -> PolyNodeTrader:
    credential = await complete_user_relayer_authorization(
        "https://trade.polynode.dev",
        os.environ["POLYNODE_API_KEY"],
        challenge,
        signature_returned_by_browser,
        address,
    )
    # Save `credential` in your backend secret manager for this address.
    return PolyNodeTrader(TraderConfig(
        polynode_key=os.environ["POLYNODE_API_KEY"],
        execution_mode=ExecutionMode.USER_OWNED,
        user_relayer_credentials=credential,
    ))
```

### Rust browser backend

```rust,no_run
use polynode::trading::{
    Address, ExecutionMode, PolyNodeTrader, TraderConfig,
    UserRelayerAuthorizationChallenge, UserRelayerCredentials,
};

async fn begin_for_browser(
    trader: &PolyNodeTrader,
    address: Address,
) -> polynode::Result<UserRelayerAuthorizationChallenge> {
    // Keep the complete challenge in short-lived backend storage and return
    // only challenge.message to the browser.
    trader.begin_user_relayer_authorization(address).await
}

async fn finish_from_browser(
    trader: &mut PolyNodeTrader,
    address: Address,
    challenge: &UserRelayerAuthorizationChallenge,
    signature_returned_by_browser: &str,
) -> polynode::Result<UserRelayerCredentials> {
...
fn restore_trader(
    polynode_key: String,
    credential: UserRelayerCredentials,
) -> polynode::Result<PolyNodeTrader> {
    PolyNodeTrader::new(TraderConfig {
        polynode_key,
        execution_mode: ExecutionMode::UserOwned,
        user_relayer_credentials: Some(credential),
        ..Default::default()
    })
}
```
...
const serviceWalletPrivateKey = process.env.POLYGON_PRIVATE_KEY;
if (!serviceWalletPrivateKey) throw new Error('Set POLYGON_PRIVATE_KEY');
...
const status = await trader.ensureReady(serviceWalletPrivateKey);
...
Use `await trader.authorizeUserOwnedExecution(serviceWalletPrivateKey)` when you
...
service_wallet_private_key = os.environ["POLYGON_PRIVATE_KEY"]
status = await trader.ensure_ready(service_wallet_private_key)
...
Use `await trader.authorize_user_owned_execution(service_wallet_private_key)` when
...
For an HSM, Privy wallet, or another caller-controlled signer, provide async
`get_address()`, `sign_typed_data(payload)`, and `sign_message(message)`
methods. `sign_message()` is required for user-owned authorization and gasless
